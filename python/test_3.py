...
if __name__ == "__main__":
    ...
    singledate = True  # calculate ephemerides at a single time at some separation from epoch.
    ...
    for i in range(1, len(dates)):
        ...
        dt, t = dtime(t)
        print "Calculating ephemerides @ or up to date %f required %f s" %(dates[i], dt)
        tReq[i] = dt / float(len(orbits)) * 1000.0
    ...
    y = np.polyval(p, dt)
    if singledate:
        pylab.plot(dt, y, 'b-', label='Single ephemeride @ date: y=%.1f + %.2f dt' %(p[1], p[0]))
    else:
        pylab.plot(dt, y, 'b-', label='Multiple ephemerides up to date: y=%.1f + %.2f dt' %(p[1], p[0]))
